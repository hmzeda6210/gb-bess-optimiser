...
during this project, organised by phase. Written to be defensible in an
interview: each finding states what was observed, how it was diagnosed, what
was ruled out, and what it means practically.
...
### Seasonal weak spot (initial hypothesis — see Phase 4 for the correction)
...
*profitability*. The full Phase 4 backtest later found the opposite pattern for
profitability — see "January weak spot" below. Forecast accuracy and financial
outcome are not the same thing; this is itself one of the project's findings.
...
2026, the walk-forward backtest produced a real loss of -£43.23 (later recomputed
as -£127.06 under periodic retraining) on a day where perfect foresight could have
made £179.86 (believed profit at forecast time was £173.22 — the model expected a
healthy profit).
...
miscalibrated) P90, could help — not yet implemented.
...
to any constant divisor (verified numerically: Sharpe computed on £ profit and on
profit/capital_base returns are identical). Converting to % returns cannot change
the ratio.
...
Capital base assumption: £3,000,000 notional (~£300k/MWh × 10MWh — an assumption,
not a sourced figure).

- Annualised return on capital: ~2.35-3.41% (arbitrage-only, before costs).
  Plausible against real BESS economics — arbitrage alone typically yields
  modest single-digit returns, which is why real projects stack capacity
  market + ancillary revenue alongside arbitrage. This is a sanity-check in
  favour of the model, not a standalone claim of strong performance.
...
### Seasonal weak spot, corrected: January, not spring

The original Phase 2 hypothesis (see above) suggested spring was the weak spot,
based on forecast accuracy across cross-validation folds. The full Phase 4
backtest — measuring actual trading profitability across all 418 real days —
found the opposite: **January (£92/day) and February (£127/day) are the two
worst-performing months**, while March (£371/day) and April (£458/day) are
among the *best*.

This was first obscured by a bug in the seasonal breakdown (grouping by month
number without year, silently merging two separate Junes/Julys — the test period
spans 14 months — into one inflated bucket). Fixed by adding a fair `profit_per_day`
metric, independent of how many times each calendar month appears in the range.

Confirmed independently three ways:
1. The corrected monthly bar chart and heatmap
2. The single largest drawdown (-£1,076) landing in the Nov 2025-Feb 2026 window
3. The 30-day rolling capture rate's deepest trough (~15-20%) landing in the same window

**Root cause:** winter's flatter day-shape (no midday solar dip, established back
in Phase 1's exploratory analysis) offers less spread to arbitrage regardless of
forecast accuracy. January's win *rate* is mid-pack (67.7%) — the battery isn't
making more mistakes in winter, it's finding smaller opportunities (avg win
£197.59, second-lowest of any month). Forecast accuracy (MAE) and trading
profitability are not the same thing — this discrepancy is itself a real finding.

### Trade costs (slippage, fees, degradation)

Cost assumptions: 0.5% slippage, £2/MWh fee, £9.4/MWh degradation (derived from a
50% cell-cost fraction of £3M capex over an 8,000-cycle rated life).

**Applying full assumed costs to the 418-day backtest turns a profit into a loss:**
...
within a wider published range, not a precise, sourced number.

This is a real, industry-consistent finding: 2-hour duration batteries doing
arbitrage-only frequently cannot cover degradation and trading costs from price
spread alone — precisely why real BESS projects stack multiple revenue streams
(balancing mechanism, frequency response, capacity market) rather than relying
on day-ahead arbitrage in isolation. This project deliberately scopes to
arbitrage-only, so this result should be read as a conservative, single-revenue-
stream figure, not a verdict on battery economics generally.

**Important caveat:** costs were applied post-hoc to an already-decided schedule,
not built into the MILP's objective function. A cost-aware optimiser would make
fewer, more selective trades — only acting on spreads wide enough to clear the
*true* breakeven (efficiency + degradation + fees combined), rather than the
efficiency-only threshold currently used. The figures above are therefore likely
a pessimistic lower bound, not the system's true achievable performance under
